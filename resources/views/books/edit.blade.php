    ...
    <div class="fixed flex flex-col items-center justify-center w-full h-full dark:bg-black bg-opacity-50">
        <div class="my-5">
            <span class="text-white text-2xl font-medium text-white">
                Por favor, Edite as informações desejáveis.
            </span>
        </div>
            ...
            <div class="mx-10">
                <div class="flex flex-col overflow-hidden bg-white rounded-lg">
                    <div class="flex flex-col px-6 py-5 bg-white">
                        <div class="mt-1">
                            <form method="post" action="{{ route('books.update', ['book' => $book->id]) }}" novalidate>
                                @method('PATCH')
                                @csrf
                                <label class="block">
                                    <span class="text-md font-medium text-gray-800">Nome</span>
                                    <input type="text" name="name" value="{{ $book->name }}" class="block w-full px-2 py-1 mt-1 text-md bg-gray-100 border-2 border-transparent rounded" />
                                    @error('name')
                                ...
                                <label class="block mt-6">
                                    ...
                                    <input type="text" name="author" value="{{ $book->author }}" class="block w-full px-2 py-1 mt-1 text-md bg-gray-100 border-2 border-transparent rounded" />
                                    @error('author')
                                    <span class="block font-medium text-brand-danger">{{ $message }}</span>
                                    @enderror
                                </label>
